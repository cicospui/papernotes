...
superMA就是ARBC的实例。ARBC实例有一个专用的提议者P，且有三个外部输入：1.提议者输入并广播消息m；2.每个节点可以输入ENDORSE信号为提案背书；3.每个节点输入ABORT信号以中止提案。
...
仅当n-f个节点已经通过ts时，节点才能在时间戳ts处执行提案。

节点在提交ts后，知道n-f个在ts之后的ARBC实例时，就会pass掉ts（也就是执行ts的提案）；如果n − f 个节点已通过 ts 但提案尚未提交，则节点在时间戳 ts 处输入提案的 ABORT 信号（相当于把ts给跨过了）。

#### 快速模式

当节点无提议请求时，广播一条跳过消息即可，不经过ARBC；
...
包括两部分：提出了一种**快速协商子协议**，一方面有RBC的功能（将多值协议减少为二进制协议）；另一方面，可以完全绕过ABA协议并终止ARBC。同时提出了**FastBA**，随机二进制协议，以绕过昂贵的硬币反转的快速通道，用于打破不同意见的僵局。

![1](figs/SuperMA示例图.png)

### 快速协商

PBFT正常情况下可以快速进行，而不用昂贵的视图转换。那么SuperMA应该也有一条快速路径，当ARBC中的提议者正确且稳定时，或在其他节点ABORT之前，接收其提议并认可他，以快速完成ARBC。

该子协议得以成立的核心是：$\textcolor{red}{若一个节点获得了一个证明：没有正确节点将会输入一个特定的二进制b值给BA，则该节点可以立即决定并输出1-b，而无需等待BA的中止。}$这很好理解，因为二进制中，如果不是b，那么就是1-b。

快速协商由三个阶段组成：Propose、Endorse、Promise（可选的）。

| 阶段          | 作用                                                         |
| ------------- | ------------------------------------------------------------ |
| Propose       | 专门的提议者向所有人广播m。                                  |
| Endorse/Abort | 接收到Endorse或Abort信号时，每个节点i广播一个带标识的信息或空信息，如果已进入BA阶段，则无需操作。 |
| Promise       | 若收到n-f个相同的endorse消息，会根据消息是m或空，将1或0输入到BA，并广播一个签名或是广播一个承诺。 |

**节点收到n-f个对m的promise时**，就可以为ARBC决定消息m了，因为此时没有正确节点会为BA输入0，同时也可也做个小优化，帮忙传播promise以快速决定。同理，收到n-f个空消息的promise时，就会为ARBC决定空。

### FastBA

FastBA的思想基于SigFree，一种不使用签名的算法。两者都是基于**轮**的协议，每一轮中节点投票支持一个或两个二进制值，每轮结束时生成一个随机数（硬币），若投票值不同，硬币可以帮助节点收敛。SigFreee最终在O（k）轮中满足1-2^-K^的概率收敛。

尽管 SigFree 在通信阶段的预期时间复杂度为 O(1)，但它总是以**概率**的方式终止。即使所有正确节点的投票值相同，仍然需要一枚幸运币，从而导致**预期有两轮**。由于概率终止，SigFree 可能**效率低下**，尤其是在 WAN 和大规模部署中。

如果 n − f 个节点承诺不投票给 1 − b 但坚持 b，则即使存在 f 个拜占庭节点，**b 仍然是唯一有待决定的候选者**。在最好的情况下，FastBA 只需要两个通信阶段就可以让正确的节点做出决定。我们将这种情况称为**快速路径**。

当某个值被n-f个节点投票选出后会被存储起来，只有**有效的值**才能被最终确定，且只有有效的值会被正确的节点携带到下一轮。

![1](figs/FastBA的快速路径和普通路径.png)

#### promise机制与快速路径

第一阶段，节点通过bval消息对b值进行投票，收到对b的n-f票后，若该节点本轮未投票给1-b，就广播对b的promise消息，以**承诺该轮中不会对1-b投票**。

第二阶段，若一个节点从n-f个节点收到了对b的promise消息，就会立即通过快速路径决定b。

#### 正常路径

如果正确的节点的投票值不同，FastBA 会采用类似于 SigFree 的正常路径。

**放大**：在bval阶段，若节点收到了f+1个b的消息后，会投票给b，以确保至少一个值有效。

若节点承诺了**相反的值1-b**，就不会进行放大。该情况下若收到了n-f个对b的bval，就会广播AUX消息。**AUX和Prom作为每个节点观察到的第一个有效值的通知**，每轮只能发送一次。

节点收到**n-f个aux或prom**消息后，广播con消息，报告其收到的有效值的集合。

收到n-f个con消息后，节点进行抛硬币并进入新的一轮，分为两种情况：1.n-f个con消息，仅包含一个有效值b，节点会将b保留到下一轮，若硬币值恰好为b，则可以决定b。2.若n-f个con消息同时包含0和1，则会抛硬币来决定。
...
灵活的推进范式可以很容易地集成到除基于链的排序之外的任何排序方法中，因为链结构并不简单地支持不同节点的并发提议，不同的排序方式如下表所示：

| 名称   | 介绍                                                         | 代表协议           |
| ------ | ------------------------------------------------------------ | ------------------ |
| 序列号 | 根据预定义的序列号对提案进行排序，用一批请求填写每个序列号。 | Paxos、PBFT        |
| 链     | 基于链的排序没有明确的排序指示符。相反，节点通过直接将其链接到前一个块来提议一个块。然后通过链结构导出块的顺序。 | HotStuff、Algorand |
| DAG    | 作为一种偏序结构，基于 DAG 的排序不具有内在的全序，而是通过将区块划分为逐轮结构并在某一轮中选择至少一个区块作为提交来驱动它 | Tusk               |
| 时间戳 | 基于时间戳的协议根据每个提案附加的时间戳对请求进行排序       | Spanner、          |

### 协议角度

SuperMA 进一步利用**承诺机制**来绕过常见情况下的随机化，不依赖于时序假设，通过假设三分之二的节点是诚实的（即群体交叉）来确保安全，无需额外的时序假设。

### 相关工作

| 名称            | 介绍                                                         |
| --------------- | ------------------------------------------------------------ |
| DispersedLedger | 将可验证的区块传播阶段与共识解耦，这样低带宽节点即使尚未收到区块，仍然可以参与协议阶段。 |
| Kauri           | 利用基于树的结构来传播提案并聚合来自大型节点的投票，以维持 HotStuff 的高吞吐量。 |
| HQ              | 将轻量级的基于仲裁的协议（无领导者）与 BFT SMR 协议相结合，使得它在存在争用和无争用的情况下都能表现良好。 |
| 可信硬件类协议  | 利用可信硬件来防止模棱两可，从而仅用n = 2f + 1个节点即可提供安全可用的服务。 |
| Eve、Rex        | 利用多核架构并在达成共识之前并发执行请求。这个方向与排序相关。（未来的方向，有点像云计算？） |
